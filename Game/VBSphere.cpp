...
void VBSphere::init(Vector3 _pos, int _size, ID3D11Device* GD, Vector3 _point)
{
	
	m_size = _size;
	//m_parent = _parent;
	m_point = _point;
	m_pos = _pos;// XMVectorAdd(_pos, m_point);
	m_scale = 3.0f  * (m_scale);
...
{	
	float time = 1.0f * GD->dt;
	m_pos.x += 10.0f * sin(time);
	m_pos.z += 10.0f * cos(time);
	//m_pos.z -= cos(time);
	//orbit(time);
	VBGO::Tick(GD);
}
